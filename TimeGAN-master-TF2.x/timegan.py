r"""Time-series Generative Adversarial Networks (TimeGAN) Codebase.
...
def timegan(ori_data, parameters):
    ...
    def embedder(X, T):
        ...
        with tf.compat.v1.variable_scope("embedder", reuse=tf.compat.v1.AUTO_REUSE):
            e_cell = tf.keras.layers.StackedRNNCells([tf.compat.v1.nn.rnn_cell.BasicRNNCell(
                num_units=hidden_dim, name=module_name) for _ in range(num_layers)])
            ...
            H = tf.keras.layers.Dense(
                hidden_dim, activation='sigmoid')(e_outputs)
        return H
    ...
    def recovery(H, T):
        ...
        with tf.compat.v1.variable_scope("recovery", reuse=tf.compat.v1.AUTO_REUSE):
            r_cell = tf.keras.layers.StackedRNNCells([tf.compat.v1.nn.rnn_cell.BasicRNNCell(
                num_units=hidden_dim, name=module_name) for _ in range(num_layers)])
            ...
            X_tilde = tf.keras.layers.Dense(
                dim, activation='sigmoid')(r_outputs)
        return X_tilde
    ...
    def supervisor(H, T):
        ...
        with tf.compat.v1.variable_scope("supervisor", reuse=tf.compat.v1.AUTO_REUSE):
            e_cell = tf.keras.layers.StackedRNNCells([tf.compat.v1.nn.rnn_cell.BasicRNNCell(
                num_units=hidden_dim, name=module_name) for _ in range(num_layers-1)])
            e_outputs, e_last_states = tf.compat.v1.nn.dynamic_rnn(
    ...
    def discriminator(H, T):
        ...
        with tf.compat.v1.variable_scope("discriminator", reuse=tf.compat.v1.AUTO_REUSE):
            d_cell = tf.keras.layers.StackedRNNCells([tf.compat.v1.nn.rnn_cell.BasicRNNCell(
                num_units=hidden_dim, name=module_name) for _ in range(num_layers)])
