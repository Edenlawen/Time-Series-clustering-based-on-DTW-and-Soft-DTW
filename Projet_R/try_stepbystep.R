...
plot(rep, type = "l")
abline(v = 681, col = "red", lwd = 3)

#-------------------------------------------------------------------------------
# Finding Feasible Windows
source("R_function/compute.searchWindows.r")

fenetresViable <- compute.searchWindows(rep, rep[104:116], 104, 80, random = FALSE)
...
PAMDTW <-
  compute.PAMClustering(fenetresViable, infoMatrixDTW[[1]], queryRef = rep[104:116])
PAMSDTW <-
  compute.PAMClustering(fenetresViable, infoMatrixSDTW[[1]], queryRef = rep[104:116])
